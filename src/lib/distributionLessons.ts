...
export const LEARN_TEXT: Record<DistributionId, string> = {
  bernoulli: 'A Bernoulli trial is one yes/no experiment: success (1) or failure (0). The single parameter p is the chance of success, so the chance of failure is 1 − p. Mean and variance are p and p(1 − p), so the spread is largest when the coin is fairest. Every binomial, geometric, and negative-binomial model is built by repeating this one-trial idea.',
  binomial: 'The binomial counts how many successes occur in a fixed number n of independent Bernoulli trials with the same success chance p. Outcomes are the integers 0, 1, …, n, and the mean is np. Use it for quizzes, defect counts, or click-throughs when each attempt is independent and the number of attempts is planned in advance. When n is large and p is not extreme, a normal curve is a useful approximation.',
  geometric: 'The geometric distribution waits for the first success. If each independent trial succeeds with probability p, X is the number of trials until that first success (1, 2, 3, …). It is memoryless: having already failed several times does not change the chance that the next trial works. The expected wait is 1/p, so rarer successes mean a longer typical wait.',
  negative_binomial: 'The negative binomial generalizes the geometric: keep running independent Bernoulli trials until you collect r successes, and count the failures (or the total trials) along the way. Extra spread appears because the stopping time itself is random. It is a standard model for overdispersed counts—sales calls until r conversions, or claims that clump more than a Poisson would allow.',
  hypergeometric: 'Hypergeometric sampling draws n items without replacement from a finite population of size N that contains K marked “success” items. Dependence matters: each draw changes the remaining mix, so the variance is smaller than a matching binomial. Use it for lot inspection, cards dealt from a deck, or any setting where the population is not huge relative to the sample.',
  poisson: 'Poisson counts events that arrive independently at a constant average rate λ in a fixed interval. The possible values are 0, 1, 2, … and both the mean and the variance equal λ. Classic pictures are arrivals, calls, typos, and rare defects. It is also the limit of a binomial when n is large, p is small, and np stays near λ.',
  discrete_uniform: 'A discrete uniform random variable puts the same probability on every integer from a through b, like a fair die. There are b − a + 1 faces, each with probability 1/(b − a + 1), and the mean sits in the middle of the range. It is the simplest finite-support model and a building block for random integers, lotteries, and equally likely seats.',
  continuous_uniform: 'A continuous uniform random variable is equally likely to land anywhere on an interval [a, b]. The density is a flat rectangle of height 1/(b − a), so probability is just the length of a sub-interval divided by (b − a). The mean is the midpoint and the variance grows with the square of the interval length. Random-number generators and “any time in this window” stories start here.',
  normal: 'The normal (Gaussian) curve is the symmetric bell centred at mean μ with spread σ. About 68%, 95%, and 99.7% of the probability sits within 1, 2, and 3 standard deviations of the mean. It appears for heights, measurement error, and averages of many independent pieces (the central limit theorem). Changing μ slides the bell; changing σ stretches or squeezes it.',
  standard_normal: 'The standard normal is the special case N(0, 1): mean 0 and standard deviation 1. Any other normal value becomes a z-score z = (x − μ)/σ, which is how tables, critical values, and “how many σ away” language work. Left-tail probabilities are Φ(z); right tails are 1 − Φ(z). Once you can read this curve, you can translate results back to the original units with x = μ + zσ.',
  lognormal: 'If the logarithm of a positive variable is normal, the variable itself is lognormal. The density lives on (0, ∞), rises quickly, then has a long right tail—typical of incomes, file sizes, and multiplicative growth. Parameters μ and σ are the mean and sd on the log scale, not on the original scale. Never treat a lognormal mean as if it were a normal mean.',
  exponential: 'The exponential models waiting time until the next event in a memoryless process with rate λ. The density starts high at 0 and decays; P(X > t) = e^{−λt}. Memoryless means the remaining wait does not depend on how long you have already waited. It is the continuous cousin of the geometric and the gap time of a Poisson process.',
  gamma: 'The gamma family covers positive, often right-skewed measurements: rainfall, insurance losses, or the wait until several Poisson events. Shape α and scale θ (or rate 1/θ) flex the curve from highly skewed to nearly symmetric. The exponential is the special case of one event, and a chi-square is a gamma with a fixed scale. Mean is αθ and variance is αθ².',
  beta: 'The beta distribution lives on the unit interval (0, 1) and is the natural model for probabilities, rates, and proportions. Two shape parameters α and β can make the density U-shaped, J-shaped, or unimodal. It is the conjugate prior for a Bernoulli/binomial success probability, so Bayesian updating has a clean closed form. Mean is α/(α + β).',
  chi_square: 'A chi-square random variable with ν degrees of freedom is the sum of ν independent squared standard normals. It is right-skewed for small ν and becomes more symmetric as ν grows. You meet it in sample variances, Pearson goodness-of-fit, and as the building block of t and F. Support is [0, ∞); mean equals ν and variance equals 2ν.',
  student_t: 'Student’s t looks like a normal bell but with heavier tails, so extreme values are more plausible. It appears when you estimate a normal mean and must replace σ with the sample sd s; degrees of freedom ν = n − 1. As ν grows, t converges to the standard normal. Variance exists only for ν > 2, which is why small samples produce wider intervals.',
  f: 'The F distribution is a ratio of two scaled chi-square variables and therefore a ratio of two sample variances (up to constants). It is right-skewed and supported on (0, ∞), with two degrees-of-freedom parameters (numerator and denominator). ANOVA, regression model comparison, and tests of equal variance all use F critical values. Large denominator df makes the curve more concentrated.',
  weibull: 'Weibull is a flexible lifetime and reliability model on (0, ∞). The shape k decides whether the hazard falls (infant failures), stays flat (random failures), or rises (wear-out). Scale λ stretches time. Engineers use it for survival curves, wind speeds, and time-to-failure; the exponential is the constant-hazard special case k = 1.',
  pareto: 'Pareto is a power-law model: most of the mass sits near a minimum x_m, then a heavy tail lets a few huge values dominate the mean. It is the classic picture of wealth, city sizes, and large insurance losses. Shape α controls tail weight; smaller α means a heavier tail. Always check whether the mean even exists before quoting an average.',
  cauchy: 'The Cauchy curve is symmetric and bell-shaped, but its tails are so heavy that the mean and variance do not exist. Samples can show wild outliers, and the sample average does not settle the way a normal average does. Location x₀ is the centre (also the median) and scale γ is the half-width at half-height. Treat it as a warning example as well as a model for certain ratios of normals.',
  logistic: 'The logistic density is symmetric like the normal but with slightly heavier tails, and its CDF is the familiar S-shaped logistic function. Location μ centres the curve and scale s controls spread. It appears in growth curves, item-response models, and logistic regression’s latent-variable story. For many teaching plots it is interchangeable with a normal, but the CDF algebra is simpler.',
  skew_normal: 'The skew-normal starts from a normal bell and adds a slant parameter α so the density can lean left or right. Location and scale still shift and stretch the curve, but mean, median, and mode no longer coincide when α ≠ 0. Use it when data look almost normal except for a consistent one-sided tail. Setting α = 0 recovers the ordinary normal.',
  laplace: 'Laplace (double exponential) has a sharp peak at the median μ and exponential tails on both sides. It is more peaked and heavier-tailed than a normal with the same scale. The maximum-likelihood location is the sample median, which is why it appears in robust estimation and some error models. Scale b sets the typical absolute deviation from μ.',
  gumbel: 'Gumbel is the classic extreme-value distribution for block maxima (the GEV with shape 0). It is slightly skewed, with a light left tail and a longer right tail. Location μ and scale β fit the typical size of the maximum in a block—yearly floods, peak temperatures, or largest claims. Minima use a reflected Gumbel.',
  inverse_gaussian: 'The inverse Gaussian (Wald) models positive, right-skewed times, especially first-passage times of Brownian motion with drift. Mean μ sets the typical passage time and shape λ controls concentration around that mean. It is a competitor to gamma and lognormal for positive data and has a simple mean–variance relationship. Larger λ makes the density more symmetric about μ.',
  stretched_beta: 'A stretched beta takes a Beta(α, β) random variable on (0, 1) and rescales it onto a chosen interval [a, b]. You keep the flexible beta shapes—U, J, or unimodal—while matching a real measurement range such as scores or temperatures. The endpoints a and b are hard bounds: no probability leaks outside. Mean and variance scale by the length (b − a).',
  mixture_normal: 'A two-component normal mixture is a weighted sum of two bells. If the means are far apart you see two modes; if they overlap you get a single peak with extra shoulders or heavier tails. Weights, means, and sds of each component are the parameters. It is a simple way to capture clustering, contamination, or “two populations in one histogram.”',
  zip: 'A zero-inflated Poisson adds an extra spike of structural zeros on top of an ordinary Poisson(λ) count model. Some units never produce events (the extra zero mass π); the rest follow Poisson. Use it for visit counts, defects, or wildlife counts with more zeros than λ can explain. Mean is (1 − π)λ, smaller than a plain Poisson with the same rate.',
  zinb: 'A zero-inflated negative binomial combines a structural-zero spike with an overdispersed count model. It handles both extra zeros and variance larger than the mean—common in ecology, insurance, and click data. Inflation π lifts P(X = 0); the negative-binomial piece handles the remaining counts. If zeros are the only problem, ZIP may suffice; if counts still clump, ZINB is the richer fit.',
  multinomial: 'The multinomial generalizes the binomial from two categories to k categories whose probabilities sum to 1. After n independent trials you record the count in each bin. It is the model for dice with several faces, survey response options, and multinomial logistic outcomes. Each margin is binomial, but the counts are negatively dependent because they share the same n.',
  dirichlet: 'The Dirichlet is a distribution on probability vectors (p₁, …, p_k) that sum to 1—the multivariate beta. Concentration parameters α_i pull mass toward categories with larger α. It is the conjugate prior for multinomial probabilities and a standard model for compositions, topic weights, and mixed membership. When all α_i = 1 the draw is uniform on the simplex.',
  empirical: 'The empirical distribution does not assume a named family. It puts mass 1/n on each observed value (or uses the eCDF F_n(x)). The “parameters” are the data themselves, so the curve updates whenever the sample changes. Use it to compare a parametric model with what the sample actually did, or when you want probabilities without fitting μ, σ, or λ.',
}

export function getDistributionLesson(id: DistributionId): DistLesson {
  const lesson = DISTRIBUTION_LESSONS[id]
  return { ...lesson, learn: LEARN_TEXT[id] ?? lesson.description }
}
