...
import {
  ...
  cltLabPath,
  loadCltProgress,
  nextIncompleteCltLab,
} from '../../lib/samplingDistributionsClt'
import { FeatureIcon, LabIcon, SamplingDistributionsHeroArt } from './icons'
import { SamplingDistributionsFrame } from './shell'
...
export function SamplingDistributionsStudioHome() {
  const [heroActive, setHeroActive] = useState(false)
  const progress = useMemo(() => loadCltProgress(), [])
  const done = progress.completed.filter((slug) => CLT_STUDIO.labs.some((lab) => lab.slug === slug))
  const pct = Math.round((done.length / CLT_STUDIO.labs.length) * 100)
  const continueLab = nextIncompleteCltLab(done)
    ...
    <SamplingDistributionsFrame>
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-xl text-[15px] leading-7 text-slate-500">
            Explore how sample statistics behave across repeated samples and why the Central Limit Theorem matters.
            See how sampling distributions arise and unlock the power of inference.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {FEATURES.map((feature) => (
              <div key={feature.title} className="clt-chip">
        ...
        <SamplingDistributionsHeroArt active={heroActive} />
      </section>

      <section className="clt-card flex flex-wrap items-center gap-4 px-5 py-4">
        <div
        ...
        <div className="min-w-[180px] flex-1">
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
            {done.length} of {CLT_STUDIO.labs.length} labs completed
          </p>
          <p className="text-xs text-slate-400">Build your sampling distributions and CLT skills step by step.</p>
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          ...
          <p className="text-xs font-semibold text-slate-400">{CLT_STUDIO.labs.length} labs · learn at your own pace</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {CLT_STUDIO.labs.map((lab, index) => {
      ...
      <section className="relative overflow-hidden rounded-[28px] bg-gradient-to-r from-slate-50 to-blue-50 px-6 py-6 dark:from-slate-900 dark:to-slate-800">
        <div className="clt-mountain pointer-events-none absolute inset-x-0 bottom-0 opacity-80" />
        <div className="relative flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <p className="max-w-md text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">
              Great work! Ready for what&apos;s next?
            </p>
            <p className="mt-1 text-xs text-slate-400">Continue building your skills with Estimation.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link to={cltLabPath(continueLab.slug)} className="clt-btn clt-btn-ghost">
              Continue learning
            </Link>
            <Link to={NEXT_STUDIO_PATH} className="clt-btn">
              Go to Estimation <ArrowRight size={15} aria-hidden />
            </Link>
          </div>
        </div>
        <p className="clt-note relative mt-4 text-right">More skills. Brighter possibilities.</p>
      </section>
    </SamplingDistributionsFrame>
  )
}
