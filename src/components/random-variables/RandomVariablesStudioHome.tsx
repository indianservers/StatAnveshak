...
import { FeatureIcon, LabIcon, RandomVariablesHeroArt } from './icons'
import { RandomVariablesFrame } from './shell'
...
export function RandomVariablesStudioHome() {
  const [heroActive, setHeroActive] = useState(false)
  const progress = useMemo(() => loadRvProgress(), [])
  const done = progress.completed.filter((slug) => RV_STUDIO.labs.some((lab) => lab.slug === slug))
  const pct = Math.round((done.length / RV_STUDIO.labs.length) * 100)
  const continueLab = nextIncompleteRvLab(done)
    ...
    <RandomVariablesFrame>
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-xl text-[15px] leading-7 text-slate-500">
            Understand discrete and continuous random variables, probability distributions, their behavior, and the
            relationships between variables through interactive labs and real-world examples.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {FEATURES.map((feature) => (
              <div key={feature.title} className="rv-chip">
        ...
        <RandomVariablesHeroArt active={heroActive} />
      </section>

      <section className="rv-card flex flex-wrap items-center gap-4 px-5 py-4">
        <div
        ...
        <div className="min-w-[180px] flex-1">
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
            {done.length} of {RV_STUDIO.labs.length} labs completed
          </p>
          <p className="text-xs text-slate-400">Start your journey into random variables.</p>
          <div className="mt-2 h-2 rounded-full bg-slate-100 dark:bg-slate-800">
            <div className="h-2 rounded-full bg-blue-600" style={{ width: `${pct}%` }} />
          </div>
        </div>
        <p className="max-w-xs text-xs leading-5 text-slate-400">
          Small steps — big insights in probability and beyond.
        </p>
      </section>
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          ...
          <p className="text-xs font-semibold text-slate-400">{RV_STUDIO.labs.length} labs · learn at your own pace</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {RV_STUDIO.labs.map((lab, index) => {
            const complete = done.includes(lab.slug)
            return (
              <article key={lab.slug} className="rv-card rv-home-card flex h-full flex-col p-5">
                ...
                <div className="mt-3 flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm leading-5 text-slate-500">{RV_HOME_COPY[lab.slug]?.blurb ?? lab.summary}</p>
                  </div>
                </div>
                <Link to={rvLabPath(lab.slug)} className="rv-btn mt-5 w-full">
      ...
      <section className="relative overflow-hidden rounded-[28px] bg-gradient-to-r from-slate-50 to-blue-50 px-6 py-6 dark:from-slate-900 dark:to-slate-800">
        <div className="rv-mountain pointer-events-none absolute inset-x-0 bottom-0 opacity-80" />
        <div className="relative flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <p className="max-w-md text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">
              Randomness today. Real understanding tomorrow.
            </p>
            <p className="mt-1 text-xs text-slate-400">Keep exploring. Random variables help make sense of the world.</p>
          </div>
          <Link to={rvLabPath(continueLab.slug)} className="rv-btn">
            Continue learning <ArrowRight size={15} aria-hidden />
          </Link>
        </div>
        <p className="rv-note relative mt-4 text-right">Data. Uncertainty. Opportunity.</p>
      </section>
    </RandomVariablesFrame>
  )
}
